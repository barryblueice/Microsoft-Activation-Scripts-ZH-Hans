...
%eline%
echo Microsoft Activation Scripts有更新，是否前往下载：  %masver%
echo ________________________________________________
echo:
...
echo [0] 无视，继续运行
echo:
call :_color %_Green% "输入对应的序号以触发不同功能 [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
echo:
echo:
echo:                     Microsoft Activation Script V2.6
echo:
echo:                                                由barryblueice汉化 
echo:
echo:       ______________________________________________________________
echo:
echo:                                 激活方式:
echo:
echo:       [序号] 功能         ^|  适用范围          ^|     有效期限
echo:
echo:       [1] HWID数字激活    ^|  Windows           ^|     永久激活 
echo:       [2] Ohook数字激活   ^|  Office            ^|     永久激活 
echo:       [3] KMS38数字激活   ^|  Windows           ^|  激活期限至2038年  
echo:       [4] 在线KMS激活     ^|  Windows / Office  ^|   激活期限180天   
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "输入对应的序号以触发不同功能 [1,2,3,4,5,6,7,8,0] :"
...
echo:             [2] 提取$OEM$文件夹
echo:
echo:             [3] 通过VBS方式检查激活状态
echo:
echo:             [4] 下载正版Windows/Office
echo:             __________________________________________________      
echo:                                                                     
echo:             [0] 回到主菜单
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "输入对应的序号以触发不同功能 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%

if %_erl%==5 goto :MainMenu
if %_erl%==4 start https://msdn.itellyou.cn/ & goto :Extras
...
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "输入对应的序号以触发不同功能:"
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回到主菜单, 选择故障排除并运行修复许可证选项。"
) else (
set "_exitmsg=退出"
set "_fixmsg=在MAS文件夹中, 运行Troubleshoot脚本并运行修复许可证选项。"
)
...
%eline%
echo 检测到未支持的OS版本 [%winbuild%].
echo HWID数字激活仅支持Windows 10/11系统。
echo 请使用在线KMS激活方式进行激活。
goto dk_done
)

if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo HWID数字激活不支持激活Windows Server系统。
echo 请使用KMS38激活方式，或使用在线KMS激活方式进行激活。
goto dk_done
)
...
echo:
echo PowerShell无法运行，正在终止进程...
echo 如果您已对Powershell应用了限制，请撤销这些更改。
echo:
echo 查看此页以获取帮助。 %mas%troubleshoot
goto dk_done
)
...
call :dk_color %Red% "启动[sppsvc]服务失败, 余下步骤将会耗费一定的时间执行..."
echo:
)
...
echo:
call :dk_color2 %_White% "     " %Green% "检查结果: %winos% 已永久激活"
call :dk_color2 %_White% "     " %Gray% "此系统已永久激活，无需执行激活脚本。"
echo ___________________________________________________________________________________________
...
echo:
echo 评估版本无法激活。
echo 你需要安装完整版的%winos%
echo:
echo 可从此处下载：
echo https://msdn.itellyou.cn/
goto dk_done
)
)
...
echo 检查OS信息                       [%winos% ^| %fullbuild% ^| %arch%]
...
if defined _int (
echo 检查联网状态            [已连接%ping_f%]
) else (
set error=1
call :dk_color %Red% "检查联网状态            未连接"
)
...
echo 正在启用Windows Script Host            [成功]
)
...
if defined updatesblock call :dk_color %Gray% "检查注册表中的更新阻止内容     [已检测到]"

if defined applist echo: %serv_e% | find /i "wuauserv" %nul% && (
call :dk_color %Blue% "Windows更新无法使用。如果你已禁用，你需要重新启用它。"
reg query HKLM\SYSTEM\CurrentControlSet\Services\wuauserv /v WubLock %nul% && call :dk_color %Blue% "Sordum Windows Update Blocker工具已用于阻止更新。"
)

reg query "HKLM\SOFTWARE\Policies\Microsoft\WindowsStore" /v DisableStoreApps %nul2% | find /i "0x1" %nul% && (
call :dk_color %Gray% "在注册表中检测Store Blocker项      [已检测到]"
)
...
call :dk_color %Red% "检查HWID的替代版本     [%altedition% 激活ID未检测到]"
)
...
echo 无法在已支持的产品列表中查找到该产品。
) else (
echo 在%SystemRoot%\System32\spp\tokens\skus\中未找到所需的License授权文件。
)
echo 请确保你使用的是最新版本的脚本。
echo %mas%
echo:
goto dk_done
)
...
call :dk_color %Blue% "[%altedition%] 版本产品密钥将用于启用HWID激活。"
echo:
)
...
if !errorlevel! EQU 0 (
echo 将Windows区域更改为美国          [成功]
) else (
call :dk_color %Red% "将Windows区域更改为美国          [失败]"
)
)
...
call :dk_color %Red% "检查ClipUp.exe文件                [未找到，正在终止进程]"
call :dk_color2 %Blue% "前往此页面以查看帮助" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "正在安装GenuineTicket.xml            [重启ClipSVC失败...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查ClipSVC tokens.dat             [未找到]"
)

%_xmlexist% (
set error=1
set rebuildinfo=1
call :dk_color %Red% "正在安装GenuineTicket.xml            [执行clipup -v -o失败]"
)

if exist "%ProgramData%\Microsoft\Windows\ClipSVC\Install\Migration\*.xml" (
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查Ticket Migration               [失败]"
)
...
if defined _perm (
echo:
call :dk_color %Green% "%winos%已使用数字授权永久激活。"
goto :dl_final
)
...
set error=1
echo:
call :dk_color %Red% "正在检查授权服务器              [连接失败]"
call :dk_color2 %Blue% "前往此页面以查看帮助" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Red% "正在删除注册项                     [失败] [!_ident!]"
) || (
...
set error=1
echo:
call :dk_color %Red% "检查网络连接问题                [已检测到] %error_code%"
call :dk_color2 %Blue% "前往此页面以查看帮助" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
if defined serv_cor (
set error=1
call :dk_color %Red% "检查Corrupt服务               [%serv_cor%]"
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查启动模式                      [%safeboot_option%] " %Blue% "[已检测到安全启动模式。将会使用普通模式运行。]"
)
...
set error=1
call :dk_color %Red% "正在检查Windows Setup阶段            [%imagestate%]"
...
set error=1
set showfix=1
call :dk_color2 %Red% "正在检测WinPE                          " %Blue% "[已检测到WinPE。将会使用普通模式运行。]"
)
...
set error=1
set wpaerror=1
call :dk_color %Red% "正在检测WPA注册失败信息             [%wpainfo%]"
) || (
echo 正在检测WPA注册表计数             [%wpainfo%]
)
...
if %dism_error% NEQ 0 (
call :dk_color %Red% "检查DISM                           [Not Responding] [%dism_error%]"
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查Eval Packages                  [Non-Eval Licenses are installed in Eval Windows]"
...
set showfix=1
call :dk_color %Blue% "%_fixmsg%"
if not defined permerror call :dk_color %Blue% "如若激活失败，请选择修复WPA注册表选项。"
)

if not defined showfix if defined wpaerror (
set showfix=1
call :dk_color %Blue% "如若激活失败则请回到主菜单，选择修复WPA注册表选项。"
)
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回到主菜单，选择故障排除并选择修复许可证选项。"
) else (
set "_exitmsg=退出"
set "_fixmsg=在MAS文件夹中，运行Troubleshoot脚本并选择修复许可证选项。"
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "输入对应的序号以触发不同功能 [1,2,3,0]"
choice /C:1230 /N
set _el=!errorlevel!
if !_el!==4  exit /b
if !_el!==3  start https://msdn.itellyou.cn/ &goto :oh_menu
...
echo:
echo 前往此页面以查看帮助. %mas%troubleshoot
goto dk_done
)
...
if defined sub_next echo Removing Office vNext Block             [成功]
...
echo Adding Reg Keys To Skip License Check   [成功]
)
...
echo Adding a Reg To Prevent Banner          [成功]
)
)
...
if %upk_result%==2 call :dk_color %Red% "Uninstalling Other/Grace Keys           [失败]"
...
call :dk_color %Gray% "Installing Missing License Files        [Office %oVer%.0 %_prod%] [成功]"
exit /b
)
...
set error=1
call :dk_color %Red% "Installing Missing License Files        [Office %oVer%.0 %_prod%] [失败]"
)
...
echo Symlinking System's sppc.dll To         ["%_hookPath%\sppcs.dll"] [成功]
echo Extracting Custom %_hook% To         ["%_hookPath%\sppc.dll"] [成功]
) else (
set error=1
call :dk_color %Red% "Symlinking Systems sppc.dll             [失败]"
call :dk_color %Red% "Extracting Custom %_hook%            [失败]"
...
set error=1
set ierror=1
call :dk_color %Red% "Modifying Hash of Custom %_hook%     [失败]"
) else (
echo Modifying Hash of Custom %_hook%     [成功]
)
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "输入对应的序号以触发不同功能 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
call :dk_color2 %_White% "     " %Gray% "无需进行激活。"
echo ___________________________________________________________________________________________
if %_unattended%==1 goto dk_done
echo:
...
if !error_code! EQU 0 (
call :dk_refresh
echo Installing KMS Client Setup Key         [%key%] [成功]
) else (
call :dk_color %Red% "Installing KMS Client Setup Key         [%key%] [失败] !error_code!"
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
call :dk_color %Red% "Adding Specific KMS Host                [LocalHost 127.0.0.2] [失败]"
)
...
call :dk_color %Red% "Copying clipup.exe File to              [%systemroot%\System32\] [失败] Aborting..."
goto :k_final
)
)
...
call :dk_color %Gray% "Stopping sppsvc Service                 [失败]"
)
...
echo:
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "Removing Specific KMS Host              [失败]"
) || (
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "输入对应的序号以触发不同功能 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=这台机器已永久激活。") else (set "ExpireMsg=该产品已永久激活。")
  ) else (
...
echo:
if defined _int (
echo      检查联网状态  [Connected]
) else (
call :_color2 %_White% "     " %Red% "检查联网状态  未连接"
)

echo %line%
echo:
echo      Dism会使用Windows Update提供修复损坏所需的文件。 
echo      该过程需要10-15分钟执行.. 
echo %line%
echo:
echo      注意： 
echo:
call :_color2 %_White% "     - " %Gray% "请确保电脑已联网。 "
call :_color2 %_White% "     - " %Gray% "请确保Windows更新服务可运行。 "
echo:
echo %line%
echo:
...
echo %line%
echo:    
echo      System File Checker将修复丢失或损坏的系统文件。 
echo      该过程需要10-15分钟执行.. 
echo:
echo:     如果SFC无法修复某些内容，请再次运行该命令，看看下次是否能够修复。 
echo:     有时，可能需要运行sfc /scannow命令3次， 
echo:     每次运行后重新启动PC,才能完全修复它能够修复的所有内容。 
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回: "
...
echo:
echo       - 此操作将会， 
echo            - 解除Windows和Office的已激活状态, 你可能需要重新激活。 
echo              如果Windows使用了主板 / OEM / 数字许可证进行激活，则无需担心这个问题。 
echo:
echo            - 清除ClipSVC、Office vNext、SPP和OSPP许可证 
echo            - 修复令牌文件夹和注册表的SPP权限 
echo            - 触发Office的修复选项 
echo:
call :_color2 %_White% "      - " %Red% "仅在必要时使用。 "
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo %line%
echo:
call :_color %Blue% "重建ClipSVC Licences"
echo:
...
timeout /t 3 %nul1%
echo:
call :dk_color2 %Blue% "前往此页面以查看帮助" %_Yellow% " %mas%change_edition_issues"
)
%line%
...
call :dk_color %Blue% "你需要重启系统以完成更改。"
echo Help: %mas%troubleshoot
